...
def prepare_task2(wb):
	...
	for wall_name, info in wall_info.items():
		for col in info:
			col_names.extend([wall_name+'_'+col+'_Maximum', wall_name+'_'+col+'_Minimum', wall_name+'_'+col+'_Mean'])
	ws.append(col_names)
	ws.freeze_panes = 'A2'
	return wb
...
def do_task3():
    ...
    for name, info in room_info.items():
        ...
        rooms[name] = sum_365day
        rooms_s[name] = sum_summer
        rooms_w[name] = sum_winter
...
def prepare_task3(wb):
	...
	ws.title = sheet
	col_names = ['File']
	base_titles = ['block-1_365', 'block-2_365', 'block-3_365', 'block-4_365', 'block-5_365', 'block-6_365', 'block-7_365', 'block-8_365'] + ['block-1_summer', 'block-2_summer', 'block-3_summer', 'block-4_summer', 'block-5_summer', 'block-6_summer', 'block-7_summer', 'block-8_summer'] + ['block-1_winter', 'block-2_winter', 'block-3_winter', 'block-4_winter', 'block-5_winter', 'block-6_winter', 'block-7_winter', 'block-8_winter']

	col_names = col_names + (base_titles * 10)
...
def prepare_task4(wb):
	...
	for room_name in room_info.keys():
		col_names.extend([room_name+'_Maximum', room_name+'_Minimum', room_name+'_Mean', room_name+'_Range'])
	ws.append(col_names)
	ws.freeze_panes = 'A2'
	return wb
...
def prepare_task5_6(wb):
	...
	for room_name, info in room_info.items():
		col_names.extend([room_name+'_Maximum', room_name+'_Minimum', room_name+'_Mean', room_name+'_correlation_EG'])
	ws.append(col_names)
	ws.freeze_panes = 'A2'
	return wb

def export_task5_6(wb, filename):
	sheet = "Task-5_6"
	ws = wb.get_sheet_by_name(sheet)
...
if __name__ == "__main__":
	...
	print "\nProcessing..\n"
	print "\nReading the Input Wall/Room Information..\n"
